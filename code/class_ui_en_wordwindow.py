...
from ui_en_wordwindow import Ui_en_wordwindow
...
#继承QWidget类和基本ui类
class ui_en_wordwindow(QtWidgets.QWidget,Ui_en_wordwindow):
	...
	def __init__(self,mainwindow): #输入参数QMainWindow对象
		super(ui_en_wordwindow,self).__init__()
		self.setupUi(self) #初始化基本ui
		self.mainwindow = mainwindow
		
		#给输入框textchange事件绑定word_judge函数
		self.en_word_input.textChanged.connect(self.word_judge) 
		self.number = 15 #设置单词库上限
		...
		#给菜单栏难度选择绑定信号发射函数
		self.mainwindow.action_en_word_low.triggered.connect(self.d1_emit)
		self.mainwindow.action_en_word_mid.triggered.connect(self.d2_emit)
		self.mainwindow.action_en_word_high.triggered.connect(self.d3_emit)
	...
	#d系列信号的发射函数，发射的同时将其它菜单栏选项设定为未选中
	def d1_emit(self):
		self.degree1_signal.emit('low')
		self.mainwindow.action_en_word_mid.setChecked(False)
		self.mainwindow.action_en_word_high.setChecked(False)
	def d2_emit(self):
		self.degree2_signal.emit('mid')
		self.mainwindow.action_en_word_low.setChecked(False)
		self.mainwindow.action_en_word_high.setChecked(False)
	def d3_emit(self):
	...
	def load_data(self,degree='low'):
		self.data = [] #初始化单词库
		try:
			word_txt = open('./source/en_word/en_word_'+degree+'.txt','r',encoding='utf8')
			count = 0
	...
	def word_judge(self):
		#获取正确文本和输入文本值
		input_text = self.en_word_input.text() 
		model_text = self.en_word_set.text()
		if self.restart:
		...
		#判断正确文本和输入文本是否相等，并做出相应应对，设置输入框最大字符数
		if model_text[:len(input_text)] != input_text:
			self.warning_set.setText('*输入错误, 注意修改')
			
			stylesheet = '#en_word_input{border:1px solid red}'
			self.setStyleSheet(stylesheet)
			
			self.en_word_input.setMaxLength(len(input_text))
		else:
			stylesheet = '#en_word_input{border:1px}'
			self.setStyleSheet(stylesheet)
			
			self.warning_set.setText('')
			self.en_word_input.setMaxLength(len(model_text))
			if model_text == input_text: #若完全相同则决定是否要切换到下一句或者是退出练习
				self.word_count+=1
				...
				self.en_word_set.setText(self.data[self.word_count])
	...
	def save_rank(self):
		...
		file_path = './source/rank/en_word_'
	
		if self.mainwindow.action_en_word_low.isChecked():
			rank_degree = 'low'
		elif self.mainwindow.action_en_word_mid.isChecked():
			rank_degree = 'mid'
		elif self.mainwindow.action_en_word_high.isChecked():
			rank_degree = 'high'
